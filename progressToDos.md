# Based on Proposal Rubric
## Cover Page
- [ ] Topic Title
- [ ] Names
- [ ] zIds of everyone
- [ ] Mentor names

## Introduction
- [ ] Reminder of topic
- [ ] Nature of teh project included (Theoretical)

## Progress 
- [ ] Detailed Explanation of everything completed
- [ ] Detailed explanation of everything started on
    - [ ] Partially (Fully) Implemented Algos
    - [ ] Experimental Results (Jethro and Laith Timing)
    - [ ] Testing (Maybe a case for the algo)
    - [ ] Further Source Readings
    - [ ] Specific Cases
- [ ] Issues or Hurdles We Had
    - [ ] How we dealt or intend to deal with them

## Assessment of Plan for Progress
- [ ] Assessment of Plan
- [ ] Revised Plan (Brief Anyways) (Literally the Same But needs to be more focused)
    - [ ] Aim to produce by the end of the project
    - [ ] Work needed to be completed to get to that point
    - [ ] More focused than what we had proposal

## Bibliography 
- [ ] Complete Reference List
- [ ] Complete In Text References

## Based On Rubric Marking
- [ ] Progress 10 Marks
    - [ ] Either Shows we Have achieved what we set out to or a plan for equivalent work has been set and made progress with.
    - [ ] Detailed Discussion on Work Completed
    - [ ] Project is well on the way to satisfactory completion
- [ ] Revised Plan 3 Marks
    - [ ] Quality Discussion of 
        - [ ] the Future Project Plan 
        - [ ] Expected Results
    - [ ] Challenges appropriately addressed as necessary
    - [ ] Reasonable strategy for continued progress
        - [ ] Presented in detail
        - [ ] Explained in detail
- [ ] Document Presentation 2 Marks
    - [ ] Citations (In-Text and Below Match)
    - [ ] Figures and Tables Labelled
        - [ ] Correct
        - [ ] Clear
    - [ ] Section and Sub-Section Heading Structure
    - [ ] Consistent Formatting
    - [ ] Pictures clear and attributed
    - [ ] Sections clearly labelled
    - [ ] No superflous material
        - [ ] No unnecessary, repetitive or unusually large figures
        - [ ] No unnecessarily lengthy text
        - [ ] No unusually wide margins
        - [ ] No uneccesary appendices
        - [ ] etc. 
    - [ ] Appropriate judgement in the placement in the body of 
        - [ ] data
        - [ ] tables
        - [ ] figures
    - [ ] Appropriate judgement in the placement in the appendices of 
        - [ ] data
        - [ ] tables
        - [ ] figures
    - [ ] Text spacing aids readibility
    - [ ] Text is clear and concise

# Based On Gerald Feedback
- [ ] To do


# Based on Paragraph Readthrough
- [ ] To do after document creation and paragraphs written etc.